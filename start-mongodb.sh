...
# Determine the correct port flag
PORT_FLAG="--port"
...
if [ -z "$MONGODB_REPLICA_SET" ]; then
  ...
  docker run --name $MONGODB_CONTAINER_NAME --publish $MONGODB_PORT:$MONGODB_PORT \
    -e MONGO_INITDB_DATABASE=$MONGODB_DB -e MONGO_INITDB_ROOT_USERNAME=$MONGODB_USERNAME \
    -e MONGO_INITDB_ROOT_PASSWORD=$MONGODB_PASSWORD --detach \
    $MONGODB_IMAGE:$MONGODB_VERSION $PORT_FLAG $MONGODB_PORT
...
docker run --name $MONGODB_CONTAINER_NAME --publish $MONGODB_PORT:$MONGODB_PORT \
  --detach $MONGODB_IMAGE:$MONGODB_VERSION $PORT_FLAG $MONGODB_PORT --replSet $MONGODB_REPLICA_SET
